...
| 0x19 | ReadDTCInformation | All 27 standard sub-functions library-formatted: DTCStatusMask validation plus the ISO wire layout for 0x01/0x02/0x0A (via `fn_dtc_list`), 0x04/0x06 framing (`fn_dtc_snapshot`/`fn_dtc_extdata`), 0x07/0x08/0x09/0x14/0x42/0x55 severity and WWH-OBD (`fn_dtc_list` + `dtc_severity_availability_mask`), 0x0B/0x0C/0x0D/0x0E first/most-recent and 0x15 permanent (`fn_dtc_list`), 0x03 snapshot identification (`fn_dtc_snapshot_ids`), 0x05 stored data and 0x16 ext-data by record number (`fn_dtc_stored_data`/`fn_dtc_extdata_by_record`), 0x0F/0x11 mirror memory, 0x12/0x13 emissions-related OBD and 0x17 user-defined memory (`fn_dtc_list_mem`), 0x18 user-defined snapshot (`fn_dtc_snapshot_mem`), 0x10/0x19 mirror and user-defined ext data (`fn_dtc_extdata_mem`). Every sub-function falls back to the raw `fn_dtc_read` hook (which receives the full request) when its structured callback is not configured — see `examples/dtc_full_coverage`. |
