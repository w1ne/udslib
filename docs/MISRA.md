...
| 11.5 | Advisory | After `uds_dtc_store_bind`, store callbacks recover a `uds_dtc_store_bind_t *` from the core's opaque `void *app_data`, then the nested store pointer. The field is kept `void *` deliberately so the protocol core carries no dependency on the optional store. |
