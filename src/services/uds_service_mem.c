...
{
    if (len < 3u) {
        return uds_send_nrc(ctx, UDS_SID_READ_MEM_BY_ADDR, UDS_NRC_INCORRECT_LENGTH); /* Incorrect Message Length Or Invalid Format */
    }
    ...
    if (ctx->config->fn_mem_read == NULL) {
        return uds_send_nrc(ctx, UDS_SID_READ_MEM_BY_ADDR, UDS_NRC_SERVICE_NOT_SUPPORTED); /* Service Not Supported */
    }

    if (size > (uint32_t)(ctx->config->tx_buffer_size - 1u)) {
        return uds_send_nrc(ctx, UDS_SID_READ_MEM_BY_ADDR, UDS_NRC_REQUEST_OUT_OF_RANGE); /* Request Out Of Range */
    }

    if (((uint64_t)addr + (uint64_t)size) > 0xFFFFFFFFu) {
        return uds_send_nrc(ctx, UDS_SID_READ_MEM_BY_ADDR, UDS_NRC_REQUEST_OUT_OF_RANGE); /* Address Overflow */
    }
