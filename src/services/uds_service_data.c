...
int uds_internal_handle_read_data_by_id(uds_ctx_t *ctx, const uint8_t *data, uint16_t len)
{
    ...
    while (i + 1u < len) {
        ...
        if (entry != NULL) {
            /* Granular Security/Session Checks */
            uint8_t sess_bit = uds_internal_get_session_bit(ctx->active_session);

            if ((entry->session_mask != 0u) && !(entry->session_mask & sess_bit)) {
                return uds_send_nrc(ctx, UDS_SID_READ_DATA_BY_ID, UDS_NRC_REQUEST_OUT_OF_RANGE);
            }
            if (entry->security_mask > ctx->security_level) {
                return uds_send_nrc(ctx, UDS_SID_READ_DATA_BY_ID, UDS_NRC_SECURITY_ACCESS_DENIED);
            }

            if ((uint32_t)tx_len + 2u + (uint32_t)entry->size > (uint32_t)ctx->config->tx_buffer_size) {
