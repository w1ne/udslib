...
int uds_internal_handle_read_data_by_id(uds_ctx_t *ctx, const uint8_t *data, uint16_t len)
{
    ...
    while (i + 1u < len) {
        ...
        if (entry != NULL) {
            if ((uint32_t)tx_len + (uint32_t)entry->size > (uint32_t)ctx->config->tx_buffer_size) {
                 return uds_send_nrc(ctx, UDS_SID_READ_DATA_BY_ID, UDS_NRC_RESPONSE_TOO_LONG); /* Response Too Long */
            }
