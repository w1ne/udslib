...
{
    if (len < 4u) {
        return uds_send_nrc(ctx, UDS_SID_ROUTINE_CONTROL, UDS_NRC_INCORRECT_LENGTH); /* Incorrect Message Length */
    }
    ...
    if (ctx->config->fn_routine_control == NULL) {
        return uds_send_nrc(ctx, UDS_SID_ROUTINE_CONTROL, UDS_NRC_CONDITIONS_NOT_CORRECT); /* Conditions Not Correct */
    }

    uint8_t *out_payload = &ctx->config->tx_buffer[4];
    uint16_t max_payload = (uint16_t)(ctx->config->tx_buffer_size - 4u);

    int written = ctx->config->fn_routine_control(ctx, type, id, &data[4], (uint16_t)(len - 4u), out_payload, max_payload);
...
int uds_internal_handle_transfer_data(uds_ctx_t *ctx, const uint8_t *data, uint16_t len)
{
    ...
    if (ctx->flash_sequence == 0u) {
        ...
        uint8_t expected = (ctx->flash_sequence == 0xFFu) ? 0x00u : (uint8_t)(ctx->flash_sequence + 1u);
