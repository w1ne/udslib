...
int uds_internal_handle_ecu_reset(uds_ctx_t *ctx, const uint8_t *data, uint16_t len)
{
    ...
    if ((sub < 0x01u) || (sub > 0x03u)) {
        return uds_send_nrc(ctx, UDS_SID_ECU_RESET, UDS_NRC_SUBFUNCTION_NOT_SUPPORTED); /* Subfunction Not Supported */
    }
...
{
    uint8_t ctrl_type = (uint8_t)(data[1] & 0x7Fu);
    uint8_t comm_type = (len >= 3u) ? data[2] : 0u;
    ...
    if (len >= 3u) {
        ...
        if ((type_nibble == 0u) || (type_nibble > 3u)) {
             return uds_send_nrc(ctx, UDS_SID_COMM_CONTROL, UDS_NRC_REQUEST_OUT_OF_RANGE); /* Request Out Of Range */
        }
    }
...
{
    if (len < 4u) {
        return uds_send_nrc(ctx, UDS_SID_CLEAR_DTC, UDS_NRC_INCORRECT_LENGTH); /* Incorrect Msg Length */
    }

    if (!ctx->config->fn_dtc_clear) {
        return uds_send_nrc(ctx, UDS_SID_CLEAR_DTC, UDS_NRC_CONDITIONS_NOT_CORRECT); /* Conditions Not Correct (No clear hook) */
    }
...
int uds_internal_handle_read_dtc_info(uds_ctx_t *ctx, const uint8_t *data, uint16_t len)
{
    ...
    if (!ctx->config->fn_dtc_read) {
        return uds_send_nrc(ctx, UDS_SID_READ_DTC_INFO, UDS_NRC_CONDITIONS_NOT_CORRECT); /* Conditions Not Correct */
    }
