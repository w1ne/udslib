...
{
    CHECK(ota_version_allows(V2, V1, 1) == 1,
          "enforce: upgrade (v2 over v1) allowed");
    return 0;
}
...
{
    CHECK(ota_version_allows(V1, V1, 1) == 1,
          "enforce: equal version (v1 over v1) allowed");
    return 0;
}
...
{
    CHECK(ota_version_allows(V1, V2, 1) == 0,
          "enforce: downgrade (v1 over v2) rejected");
    return 0;
}
...
{
    CHECK(ota_version_allows(V1, V2, 0) == 1,
          "no-enforce: downgrade (v1 over v2) allowed");
    return 0;
}
...
{
    CHECK(ota_version_allows(V2, V1, 0) == 1,
          "no-enforce: upgrade (v2 over v1) allowed");
    return 0;
}
...
static int test_zero_floor_allows_any(void)
{
    ...
    CHECK(ota_version_allows(0u, 0u, 1) == 1,
          "enforce: zero candidate over zero floor allowed");
    return 0;
}
...
{
    CHECK(OTA_ANTIROLLBACK_ENFORCE == 1,
          "default OTA_ANTIROLLBACK_ENFORCE == 1 (enforced)");
