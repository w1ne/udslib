...
"""
mkimage.py — pack a raw app .bin into a valid, ECDSA-signed OTA image.
...
Output layout (matches ota_image.h / app_jump.c):
    ...
    0x400+N           64       signature  = ECDSA-P256 raw r||s over SHA-256(payload)
...
Image authenticity (secure boot):
  ...
  Signature: raw 64-byte r||s (32+32, big-endian), appended after the payload.
The bootloader verifies r,s as MPIs against the baked public key with
mbedtls_ecdsa_verify (no ASN.1/DER on the device). The private key used here
(app/signing_key_dev.pem) is a DEMO key committed for the example only — a real
product keeps the signing key in an HSM / offline and NEVER in the repo.
"""
...
OTA_SIG_SIZE = 64                   # raw r||s, 32 + 32 big-endian


def sign_payload(payload: bytes, key_path: str) -> bytes:
    """
    Sign SHA-256(payload) with the EC P-256 private key in key_path (PEM).
    Returns the raw 64-byte r||s signature (the cryptography lib emits DER, so
    decode it back to integers and re-encode as fixed-width big-endian).
    """
    with open(key_path, "rb") as f:
        priv = serialization.load_pem_private_key(f.read(), password=None)
    if not isinstance(priv, ec.EllipticCurvePrivateKey) or \
            priv.curve.name != "secp256r1":
        raise ValueError(f"{key_path}: not a secp256r1 private key")
...
def build_image(payload: bytes, version: int, key_path: str,
    ...
    Build a complete, signed OTA image:
      [0x400-byte header region][payload][64-byte ECDSA-P256 signature].
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="EC P-256 private key (PEM) used to sign the payload "
