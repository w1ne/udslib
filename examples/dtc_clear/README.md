...
Wire it with `uds_dtc_bind` (custom ops path — not the reference store):

```c
const uds_dtc_ops_t ops = {
    .app_data = &ecu,          /* recovered via ctx->config->app_data */
    .clear    = ecu_clear_dtc, /* ClearDiagnosticInformation (0x14) */
};
...
- `../dtc_store` — the **opt-in reference store** via `uds_dtc_store_bind`
  implements clear for you so you don't write the table by hand.
