...
#if defined(CONFIG_LIBUDS_TRANSPORT_NATIVE)
...
#elif defined(CONFIG_LIBUDS_TRANSPORT_FALLBACK)
extern int uds_zephyr_tp_fallback_init(struct uds_ctx *uds_ctx, uint32_t rx_id, uint32_t tx_id);
#endif

/* Buffers */
static uint8_t uds_rx_buf[CONFIG_LIBUDS_RX_BUFFER_SIZE];
static uint8_t uds_tx_buf[CONFIG_LIBUDS_TX_BUFFER_SIZE];
...
int main(void) {
    ...
    while (1) {
#if defined(CONFIG_LIBUDS_TRANSPORT_NATIVE)
        uint8_t frame[CONFIG_LIBUDS_MAX_SDU_SIZE];
