...
### Added
- **Standardized Data Identifier (DID) constants** (`uds/uds_did_ids.h`): named `UDS_DID_*` macros for the full ISO 14229-1 identification block (0xF180–0xF19F) — VIN, ECU serial number, software/data fingerprints, manufacturer/supplier part and version numbers, calibration and programming dates, ODX file, etc. Use them in a `uds_did_table_t` so a DID entry reads `{UDS_DID_VIN, 17, ...}` instead of a bare hex literal. The constants only name *which* datum is addressed; the data behind each DID stays application-owned (the library does not store VINs, serials, etc.). `examples/host_sim` updated to demonstrate the named constants. (#69)
