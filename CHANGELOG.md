...
### Added
- **ISO-TP configurable frame padding**: `uds_tp_isotp_set_pad_byte()` sets the byte used to fill unused bytes in every transmitted frame (SF, FF, CF, FC). ISO 15765-2 leaves the padding value implementation-defined; the default stays 0x00 (prior behavior), and bus partners or test tools that require 0xAA/0xCC can now select it. (#67)
